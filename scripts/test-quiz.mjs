...
assert.equal(chapter.length, 100);
assert.equal(chapter.filter((q) => q.responseType === "number").length, 20);
assert.deepEqual([0, 1, 2, 3].map((i) => chapter.filter((q) => q.responseType === "choice" && q.answer === i).length), [20, 20, 20, 20]);
assert.equal(new Set(chapter.map((q) => q.prompt)).size, 100);
assert.deepEqual(bank.topics.filter((t) => t.id.startsWith("c1-")).map((t) => chapter.filter((q) => q.topic === t.id).length), [12, 10, 14, 14, 12, 8, 18, 12]);
...
assert.equal(numericKeys.size, 20);
...
console.log(`OK: 100 questions, 20 numeric + ${calculatedChoices.size} choice calculations, balanced keys, numeric parsing, grading and persistence.`);
